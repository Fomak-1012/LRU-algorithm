...
// --------------------------
	/**
	 * follows are constructors and destructors
	...
	*/
	...
	public:
		...
		*/
		T &operator*() const {
			if(p == nullptr || p->pre == nullptr || p->nxt == nullptr || p->val == nullptr)
				throw "invalid";
			return *(p->val);//注意返回值
		}
        /**
	...
	public:
		...
		const T &operator*() const {
			if(p == nullptr || p->nxt == nullptr || p->pre == nullptr || p->val == nullptr)
				throw "invalid";
			return *(p->val);
		}
...
// --------------------------
	...
	*/
	...
	hashmap(const hashmap &other) : capacity(other.capacity), size(0), load_factor(other.load_factor) {
    buckets = new double_list<value_type>[capacity];
    ...
    std::swap(this->buckets, tmp.buckets);
    std::swap(this->load_factor, tmp.load_factor);
...
// --------------------------
        ...
        */
		iterator(size_t idx, typename double_list<value_type>::iterator it, hashmap* ptr) 
			 : index(idx), iter(it), map_ptr(ptr) {}
		iterator(const iterator &t)
			 : index(t.index), iter(t.iter), map_ptr(t.map_ptr) {}
...
// --------------------------
    ...
    // --------------------------
		 ...
		 */
		iterator &operator--() {
			if (map_ptr == nullptr || lit == map_ptr->global_list.begin()) throw "invalid";
            --lit; 
			return *this;
		}
		...
		*/
		value_type &operator*() const {
			if (map_ptr == nullptr || lit == map_ptr->global_list.end()) throw "star invalid";
            return *lit;
		}
		value_type *operator->() const noexcept {
			return &(*lit);
...
public:
	...
	*/
	T & at(const Key &key) {
        ...
        if(hash_it == base_hashmap::end()) throw "index out of bound";

        list_iterator lit = (*hash_it).second;
        value_type val = *lit;

        // LRU: remove from current position, insert to head
        global_list.erase(lit);
        global_list.insert_head(val);

        // update hashmap to point to new head
	...
	T & operator[](const Key &key) {
        ...
        if(hash_it != base_hashmap::end()) {
            return at(key); // reuse at() logic for LRU move
        }
	...
	*/
	void remove(iterator pos) {
		if(pos == end() || pos.map_ptr != this) throw "invalid remove";
